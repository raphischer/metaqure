...
def extract(data, targets):
    ...
    results = {
        ...
        'onehot_bal_mean': np.mean(oh_bal) if len(oh_bal) > 0 else np.nan,
        'onehot_bal_std': np.std(oh_bal) if len(oh_bal) > 0 else np.nan,
        'onehot_bal_max': np.max(oh_bal) if len(oh_bal) > 0 else np.nan,
        'onehot_bal_min': np.min(oh_bal) if len(oh_bal) > 0 else np.nan
    }
