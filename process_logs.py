...
if __name__ == "__main__":
    ...
    baselines.reset_index(drop=True).to_pickle(os.path.join(args.db_dir, f'baselines.pkl'))
    complete.reset_index(drop=True).to_pickle(os.path.join(args.db_dir, f'complete.pkl'))

    subset = complete[complete['dataset'].isin(pd.unique(complete['dataset'])[5:15].tolist() + ['credit-g'])]
    subset.reset_index(drop=True).to_pickle(os.path.join(args.db_dir, f'subset.pkl'))
